...
try:
    ...
    HAVE_STRATEGY = True
    _STRATEGY_ERR = ""
except Exception as _e:  # noqa: BLE001 - missing file etc. -> autonomy just disabled
    HAVE_STRATEGY = False
    _STRATEGY_ERR = str(_e)
...
class App:
    def __init__(self, mode, weights, conf, yolo_interval):
        ...
        if not HAVE_STRATEGY:
            print(
                f"[autonomy] strategy_full not importable ({_STRATEGY_ERR}); "
    ...
    def draw(self):
        ...
        if self.autonomous:
            ...
            auto_txt = "AUTO: off"
            auto_col = (150, 150, 150)
        s.blit(self.f_small.render(auto_txt, True, auto_col), (640, 626))
